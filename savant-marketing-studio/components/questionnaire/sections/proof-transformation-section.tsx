'use client';

import { useState } from 'react';
import QuestionWrapper from '../question-types/question-wrapper';
import LongTextQuestion from '../question-types/long-text-question';
import { FileUploadQuestion } from '../question-types/file-upload-question';
import SectionContainer from './section-container';
import { UseQuestionnaireFormReturn } from '@/lib/questionnaire/use-questionnaire-form';
import { HelpPanel, HelpContent } from '../help-system';

interface Props {
  clientId: string;
  questionnaireForm: UseQuestionnaireFormReturn;
}

export default function ProofTransformationSection({ clientId, questionnaireForm }: Props) {
  const {
    formData,
    updateQuestion,
    markQuestionCompleted,
    completedQuestions,
  } = questionnaireForm;

  const [helpOpen, setHelpOpen] = useState(false);
  const [currentHelpQuestion, setCurrentHelpQuestion] = useState(25);

  const openHelp = (questionNumber: number) => {
    setCurrentHelpQuestion(questionNumber);
    setHelpOpen(true);
  };

  // Calculate section progress
  const sectionQuestions = ['q25', 'q26', 'q27', 'q28', 'q29'];
  const answeredCount = sectionQuestions.filter(q => 
    completedQuestions.has(q)
  ).length;

  return (
    <>
      <HelpPanel isOpen={helpOpen} onClose={() => setHelpOpen(false)}>
        <HelpContent questionNumber={currentHelpQuestion} />
      </HelpPanel>

      <SectionContainer
        sectionNumber={6}
        title="Proof & Transformation"
        description="Demonstrate your track record and results"
        estimatedTime="9 minutes"
        currentProgress={{ answered: answeredCount, total: 5 }}
      >
        <QuestionWrapper
          questionNumber={25}
          questionText="Share a powerful transformation story"
          isRequired={true}
          onHelpClick={() => openHelp(25)}
          estimatedTime="3 min"
        >
          <LongTextQuestion
            value={formData.proof_transformation.q25_transformation_story}
            onChange={(val) => updateQuestion('q25', val)}
            onBlur={() => markQuestionCompleted('q25')}
            placeholder="Sarah came to us $50K in debt and working 80 hours/week. After 90 days..."
            minLength={100}
            maxLength={2000}
          />
        </QuestionWrapper>

        <QuestionWrapper
          questionNumber={26}
          questionText="What measurable results do you deliver?"
          isRequired={true}
          onHelpClick={() => openHelp(26)}
          estimatedTime="2 min"
        >
          <LongTextQuestion
            value={formData.proof_transformation.q26_measurable_results}
            onChange={(val) => updateQuestion('q26', val)}
            onBlur={() => markQuestionCompleted('q26')}
            placeholder="Average client ROI: 3.2x in 90 days. 89% achieve positive ROI within 60 days..."
            minLength={50}
            maxLength={1000}
          />
        </QuestionWrapper>

        <QuestionWrapper
          questionNumber={27}
          questionText="What credentials or recognition do you have? (Optional)"
          onHelpClick={() => openHelp(27)}
          estimatedTime="1 min"
        >
          <LongTextQuestion
            value={formData.proof_transformation.q27_credentials}
            onChange={(val) => updateQuestion('q27', val)}
            onBlur={() => markQuestionCompleted('q27')}
            placeholder="Featured in Forbes, Inc, Entrepreneur. Certified by..."
            maxLength={500}
          />
        </QuestionWrapper>

        <QuestionWrapper
          questionNumber={28}
          questionText="What guarantees do you offer?"
          isRequired={true}
          onHelpClick={() => openHelp(28)}
          estimatedTime="1 min"
        >
          <LongTextQuestion
            value={formData.proof_transformation.q28_guarantees}
            onChange={(val) => updateQuestion('q28', val)}
            onBlur={() => markQuestionCompleted('q28')}
            placeholder="100% money-back guarantee if you don't see 20+ qualified leads in 90 days..."
            minLength={30}
            maxLength={500}
          />
        </QuestionWrapper>

        <QuestionWrapper
          questionNumber={29}
          questionText="Upload proof materials (optional)"
          isRequired={false}
          onHelpClick={() => openHelp(29)}
          estimatedTime="2 min"
        >
          <FileUploadQuestion
            value={formData.proof_transformation.q29_proof_assets || []}
            onChange={(files) => {
              updateQuestion('q29', files);
              if (files.length > 0) markQuestionCompleted('q29');
            }}
            label="Proof Materials"
            description="Upload testimonials, case studies, screenshots, or any proof materials (optional)"
            maxFiles={5}
            maxSizeInMB={10}
            acceptedTypes={['image/*', 'application/pdf', '.doc', '.docx']}
          />
        </QuestionWrapper>
      </SectionContainer>
    </>
  );
}
